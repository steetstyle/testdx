import { GlobalVariables, ScenarioVariables, DistributionParams, DistributionType } from './types';
...
export class ScenarioVariablesBuilder {
  ...
  private resolveVariableValue(value: string | number | boolean | string[] | number[] | Record<string, any>): string | number | boolean | string[] | number[] | Record<string, any> {
    ...
    if (Array.isArray(value)) {
      return value.map(v => typeof v === 'string' ? this.resolveVariableValue(v) : v);
    }
